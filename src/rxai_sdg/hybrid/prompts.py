...
def task_description_reasoning_completion_single(
    ...
    if system_prompt is not None:
        memory_str = f'''Provided system prompt is an instruction that produced the specific response. It should not be interpreted as a prompt for the reasoning generated by the model.
However, when generating the reasoning, keep this system prompt in mind, as it serves as the underlying implication from which the response follows.\n## SYSTEM PROMPT \n{system_prompt}''' + memory_str
...
4. Target approximately {target_tokens} tokens. But do not keep token count in mind when generating the reasoning, it is just a rough target to aim for, so focus on the quality of the reasoning and not on the token count. If it is not necessary to generate a long reasoning to connect the query to the answer, then generate a shorter reasoning, but if it is necessary to generate a long reasoning to connect the query to the answer, then generate a longer reasoning. The quality of the reasoning is more important than the token count.
...
8. Be concise, avoid unnecessary verbosity while maintaining clarity and depth in reasoning **IMPORTANT**
{'9. Strictly follow the instructions in the system prompt for reasoning blocks, ensure they are fulfilled add some reference to your system prompt in your responses' if system_prompt is not None else ''}
...
def task_description_reasoning_completion_all(
    ...
    if system_prompt is not None:
        system_prompt_instruction = f'''Provided system prompt is an instruction that produced the specific responses. It should not be interpreted as a prompt for the reasoning generated by the model.
However, when generating the reasoning, keep this system prompt in mind, as it serves as the underlying implication from which the responses follow.\n## SYSTEM PROMPT \n{system_prompt}'''
...
4. Target approximately {target_tokens_per_think} tokens per thinking block. But do not keep token count in mind when generating the reasoning, it is just a rough target to aim for, so focus on the quality of the reasoning and not on the token count. If it is not necessary to generate a long reasoning to connect the query to the answer, then generate a shorter reasoning, but if it is necessary to generate a long reasoning to connect the query to the answer, then generate a longer reasoning. The quality of the reasoning is more important than the token count.
...
8. Be concise, avoid unnecessary verbosity while maintaining clarity and depth in reasoning **IMPORTANT**
{'9. Strictly follow the instructions in the system prompt for reasoning blocks, ensure they are fulfilled' if system_prompt is not None else ''}
